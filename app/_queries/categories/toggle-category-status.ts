...
import { Category, ToggleCategoryStatusInput } from "@/lib/types/category";
...
async function toggleCategoryStatus(
  category_id: string,
  category_props: ToggleCategoryStatusInput,
): Promise<ApiResponse<Category>> {
  const resp = await apiClient<Category, ToggleCategoryStatusInput>(
    `${API_ADMIN_PREFIX}/categories/${category_id}/status`,
    {
      method: "patch",
...
export function useToggleCategoryStatus(category_id: string) {
  ...
  >({
    mutationFn: (category_props) =>
      toggleCategoryStatus(category_id, category_props),
    onSuccess: () => {
